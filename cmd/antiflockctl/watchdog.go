...
func runWatchdogAdmit(arguments []string, stdout, stderr io.Writer) int {
	...
	if err != nil || strings.TrimSpace(string(source)) == "" {
		fmt.Fprintln(stderr, "admit requires a non-empty bounded regular source-file")
		return 2
	}
	endpoint, token, client, err := watchdogConnection(*baseURL, *tokenFile, *caFile, *timeout)
	if err != nil {
		fmt.Fprintf(stderr, "watchdog connection: %v\n", err)
		return 2
	}
	payload := map[string]any{"nodeId": *nodeID, "source": string(source), "bindingId": *bindingID, "operationId": *operationID}
	if err := submitWatchdogJSON(context.Background(), client, endpoint+"/v1/watchdogs", token, payload, http.StatusCreated, stdout); err != nil {
		fmt.Fprintf(stderr, "watchdog admission failed: %v\n", err)
		return 1
	}
	return 0
}
...
func runWatchdogRun(arguments []string, stdout, stderr io.Writer) int {
	...
	if err := submitWatchdogJSON(context.Background(), client, endpoint+"/v1/watchdogs/"+url.PathEscape(*programID)+"/run", token, payload, http.StatusOK, stdout); err != nil {
		fmt.Fprintf(stderr, "watchdog run failed: %v\n", err)
		return 1
	}
	return 0
}
...
func runWatchdogRunOpenFindings(arguments []string, stdout, stderr io.Writer) int {
	...
	if err := submitWatchdogJSON(context.Background(), client, endpoint+"/v1/watchdogs/"+url.PathEscape(*programID)+"/run-open-findings", token, map[string]any{}, http.StatusOK, stdout); err != nil {
		fmt.Fprintf(stderr, "watchdog Core-finding run failed: %v\n", err)
		return 1
	}
	return 0
}
...
func watchdogConnection(rawURL, tokenPath, caPath string, timeout time.Duration) (string, string, *http.Client, error) {
	if timeout < 100*time.Millisecond || timeout > time.Minute {
		return "", "", nil, errors.New("timeout must be from 100ms through one minute")
	}
...
func watchdogBaseURL(raw string) (string, error) {
	...
	if err != nil || value.Scheme == "" || value.Host == "" || value.User != nil || value.RawQuery != "" || value.Fragment != "" || (value.Scheme != "https" && value.Scheme != "http") {
		return "", errors.New("core URL must be an absolute HTTP(S) URL without credentials, query, or fragment")
	}
	if value.Scheme == "http" && !watchdogLoopback(value.Hostname()) {
		return "", errors.New("core requires HTTPS outside loopback")
	}
...
func readWatchdogToken(path string) (string, error) {
	...
	if len(token) < 32 {
		return "", errors.New("token-file must contain a bearer token of at least 32 bytes")
	}
	return token, nil
}
...
func readRegularBoundedFile(path string, maximum int64, private bool) ([]byte, error) {
	...
	if err != nil || !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Size() < 0 || info.Size() > maximum || (private && info.Mode().Perm() != 0o600) {
...
func submitWatchdogJSON(ctx context.Context, client *http.Client, endpoint, token string, payload map[string]any, expectedStatus int, stdout io.Writer) error {
	...
	if err != nil {
		return errors.New("encode watchdog request")
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint, bytes.NewReader(encoded))
	if err != nil {
		return errors.New("build watchdog request")
	}
	...
	if err != nil {
		return errors.New("submit watchdog request")
	}
	defer response.Body.Close()
	content, err := io.ReadAll(io.LimitReader(response.Body, maximumWatchdogReply+1))
	if err != nil || len(content) > maximumWatchdogReply {
		return errors.New("read bounded watchdog response")
	}
	...
	if err := decoder.Decode(&value); err != nil {
		return errors.New("core returned invalid watchdog JSON")
	}
	if err := requireWatchdogEOF(decoder); err != nil {
		return err
	}
	pretty, err := json.MarshalIndent(value, "", "  ")
	if err != nil {
		return errors.New("format watchdog response")
	}
	if _, err := fmt.Fprintln(stdout, string(pretty)); err != nil {
		return errors.New("write watchdog response")
	}
	return nil
}
