...
func TestEnrollRetrievesMatchingCertificateOverPrivateCA(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(directory, "node.seed"), privateKey.Seed(), 0o600); err != nil {
		t.Fatal(err)
	}
	state := `{"schemaVersion":"antiflock.agent-enrollment/v1","nodeId":"agent-lab-1","requestId":"enroll-test"}`
	if err := os.WriteFile(filepath.Join(directory, "enrollment.json"), []byte(state), 0o600); err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC()
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		content, err := (protojson.MarshalOptions{}).Marshal(response)
		if err != nil {
	...
	if err != nil {
		t.Fatalf("enroll: %v; stderr=%s", err, stderr.String())
	}
	var output enrollmentOutput
	if err := json.Unmarshal(stdout.Bytes(), &output); err != nil {
		t.Fatal(err)
	}
	if output.Status != "approved-ready-to-submit" || output.CertificatePath != filepath.Join(directory, "node.pem") {
		t.Fatalf("enrollment output = %#v", output)
	}
	content, err := os.ReadFile(output.CertificatePath)
	if err != nil {
		t.Fatal(err)
	}
	block, _ := pem.Decode(content)
	if block == nil || string(block.Bytes) != string(certificateDER) {
		t.Fatal("approved certificate was not installed")
	}
}
...
func TestStatusReportsPrivateIdentityAndQueueMetadata(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(stateDirectory, "node.seed"), privateKey.Seed(), 0o600); err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC()
	...
	if err := os.WriteFile(filepath.Join(stateDirectory, "node.pem"), pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: certificateDER}), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	defer queue.Close()
	var stdout, stderr bytes.Buffer
	if err := run(context.Background(), []string{"status", "--node-id", "node-status", "--state-dir", stateDirectory, "--queue-dir", filepath.Join(directory, "queue")}, &stdout, &stderr); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(stdout.String(), `"identity": "ready"`) || !strings.Contains(stdout.String(), `"retainedEvents": 0`) {
