...
type Result struct {
	...
	StateDirectory      string
	Status              antiflockv1.EnrollmentStatus
...
func Submit(ctx context.Context, config Config) (Result, error) {
	...
	endpoint, err := enrollmentEndpoint(config.Endpoint)
	if err != nil {
		return Result{}, err
	}
	if len(strings.TrimSpace(config.Token)) < 32 {
		return Result{}, errors.New("enrollment token is required")
	}
	if !canonical(config.NodeID, 128) || strings.TrimSpace(config.DisplayName) == "" || len(config.DisplayName) > 128 {
		return Result{}, errors.New("enrollment node id and display name are required")
	}
	clock := config.Clock
	if clock == nil {
		clock = func() time.Time { return time.Now().UTC() }
	}
	privateKey, persisted, err := ensureIdentity(config.StateDirectory, config.NodeID)
	if err != nil {
		return Result{}, err
	}
	now := clock().UTC()
	if now.IsZero() {
		return Result{}, errors.New("enrollment clock is invalid")
	}
	...
	proof, err := coreenrollment.ProofMessage(request)
	if err != nil {
		return Result{}, err
	}
	request.ProofOfPossession = ed25519.Sign(privateKey, proof)
	body, err := (protojson.MarshalOptions{UseProtoNames: false}).Marshal(request)
	if err != nil {
	...
	httpRequest, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint+"/v1/enrollment/nodes", bytes.NewReader(body))
	if err != nil {
		return Result{}, errors.New("build enrollment request")
	}
	...
	if readErr != nil || response.StatusCode != http.StatusAccepted {
		return Result{}, errors.New("enrollment request was not accepted by Core")
	}
	var output antiflockv1.EnrollNodeResponse
	if err := (protojson.UnmarshalOptions{DiscardUnknown: false}).Unmarshal(content, &output); err != nil || output.GetEnrollment() == nil || output.GetEnrollment().GetId() == "" {
		return Result{}, errors.New("decode Core enrollment response")
	}
	...
	if output.GetEnrollment().GetStatus() == antiflockv1.EnrollmentStatus_ENROLLMENT_STATUS_APPROVED {
		if len(certificateDER) == 0 || !certificateMatches(privateKey, certificateDER) {
			return Result{}, errors.New("approval from Core did not return the enrolled node certificate")
		}
	} else if len(certificateDER) != 0 {
		return Result{}, errors.New("certificate returned by Core before enrollment approval")
	}
...
func SaveApprovedCertificate(seedPath, certificatePath string, certificateDER []byte) error {
	privateKey, err := loadSeed(seedPath)
	if err != nil {
		return err
	}
	if !certificateMatches(privateKey, certificateDER) {
		return errors.New("approved certificate does not match enrolled seed")
	}
	if strings.TrimSpace(certificatePath) == "" {
		return errors.New("approved certificate path is required")
	}
	parent := filepath.Dir(certificatePath)
	info, err := os.Lstat(parent)
	if err != nil || !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
		return errors.New("approved certificate directory must exist and not be a symlink")
	}
	...
	if existing, err := os.Lstat(certificatePath); err == nil {
		if !existing.Mode().IsRegular() || existing.Mode()&os.ModeSymlink != 0 || existing.Mode().Perm() != privateFileMode || existing.Size() == 0 || existing.Size() > 1<<20 {
			return errors.New("approved certificate file is not private and regular")
		}
		previous, err := os.ReadFile(certificatePath)
		if err != nil || !bytes.Equal(previous, content) {
			return errors.New("approved certificate already exists with different contents")
		}
		return nil
	} else if !errors.Is(err, os.ErrNotExist) {
		return errors.New("inspect approved certificate file")
	}
...
func ensureIdentity(directory, nodeID string) (ed25519.PrivateKey, state, error) {
	if strings.TrimSpace(directory) == "" {
		return nil, state{}, errors.New("enrollment state directory is required")
	}
	if err := ensurePrivateDirectory(directory); err != nil {
		return nil, state{}, err
	}
	seedPath := filepath.Join(directory, "node.seed")
	...
	if _, err := os.Lstat(seedPath); err == nil {
		...
		persisted, err := loadState(statePath, nodeID)
		if err != nil {
			return nil, state{}, err
		}
		return privateKey, persisted, nil
	} else if !errors.Is(err, os.ErrNotExist) {
		return nil, state{}, errors.New("inspect enrollment seed")
	}
	if _, err := os.Lstat(statePath); err == nil {
		return nil, state{}, errors.New("enrollment state exists without a seed")
	} else if !errors.Is(err, os.ErrNotExist) {
		return nil, state{}, errors.New("inspect enrollment state")
	}
	_, privateKey, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		return nil, state{}, errors.New("generate enrollment seed")
	}
	requestID, err := randomID("enroll")
	if err != nil {
		return nil, state{}, err
	}
	...
	if err != nil {
		return nil, state{}, errors.New("encode enrollment state")
	}
	if err := writePrivateNew(seedPath, privateKey.Seed()); err != nil {
		return nil, state{}, err
	}
	if err := writePrivateNew(statePath, content); err != nil {
		return nil, state{}, err
	}
	return privateKey, persisted, nil
}
...
func ensurePrivateDirectory(directory string) error {
	if err := os.MkdirAll(directory, privateDirectoryMode); err != nil {
		return errors.New("create enrollment state directory")
	}
	info, err := os.Lstat(directory)
	if err != nil || !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
		return errors.New("enrollment state directory must be a real directory")
	}
	if err := os.Chmod(directory, privateDirectoryMode); err != nil {
		return errors.New("protect enrollment state directory")
	}
	return nil
}
...
func loadSeed(path string) (ed25519.PrivateKey, error) {
	...
	if err != nil || !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != privateFileMode || info.Size() != ed25519.SeedSize {
		return nil, errors.New("enrollment seed must be a private regular seed file")
	}
	content, err := os.ReadFile(path)
	if err != nil || len(content) != ed25519.SeedSize {
		return nil, errors.New("read enrollment seed")
	}
...
func writePrivateNew(path string, content []byte) error {
	...
	if err != nil {
		return errors.New("create private enrollment state")
	}
	if _, err := file.Write(content); err != nil {
		file.Close()
		return errors.New("write private enrollment state")
	}
	if err := file.Sync(); err != nil {
		file.Close()
		return errors.New("sync private enrollment state")
	}
	if err := file.Close(); err != nil {
		return errors.New("close private enrollment state")
	}
	return nil
}
...
func loadState(path, nodeID string) (state, error) {
	info, err := os.Lstat(path)
	if err != nil || !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != privateFileMode || info.Size() == 0 || info.Size() > maxStateBytes {
		return state{}, errors.New("enrollment state must be a private regular file")
	}
	content, err := os.ReadFile(path)
	if err != nil {
		return state{}, errors.New("read enrollment state")
	}
	var value state
	if json.Unmarshal(content, &value) != nil || value.SchemaVersion != stateSchema || value.NodeID != nodeID || !canonical(value.RequestID, 128) {
		return state{}, errors.New("enrollment state is invalid")
	}
	return value, nil
}
...
func enrollmentEndpoint(raw string) (string, error) {
	...
	if err != nil || value.Scheme == "" || value.Host == "" || value.User != nil || value.RawQuery != "" || value.Fragment != "" || (value.Scheme != "https" && value.Scheme != "http") {
		return "", errors.New("enrollment Core URL must be absolute HTTP(S)")
	}
	if value.Scheme == "http" && !loopback(value.Hostname()) {
		return "", errors.New("enrollment requires HTTPS outside loopback")
	}
