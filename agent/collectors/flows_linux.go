...
func (collector *LinuxCollector) collectFlows(observedAt time.Time) ([]*antiflockv1.FlowObservation, []string) {
	...
	for _, table := range tables {
		...
		if err != nil {
			reasons = append(reasons, "AF-COLLECTOR-FLOW-UNAVAILABLE")
			continue
		}
		if len(content) > maximumSourceSize {
			reasons = append(reasons, "AF-COLLECTOR-FLOW-OVERSIZED")
			continue
		}
		values, err := parseProcSocketTable(content, table.protocol, table.label, observedAt, maximumFlowRecords-len(flows))
		if err != nil {
			reasons = append(reasons, "AF-COLLECTOR-FLOW-INVALID")
			continue
		}
		flows = append(flows, values...)
		if len(flows) >= maximumFlowRecords {
			reasons = append(reasons, "AF-COLLECTOR-FLOW-LIMITED")
			break
		}
	}
...
func parseProcSocketTable(content []byte, protocol antiflockv1.TransportProtocol, label string, observedAt time.Time, limit int) ([]*antiflockv1.FlowObservation, error) {
	if limit < 0 {
		return nil, errors.New("flow record limit is invalid")
	}
	if limit == 0 {
		return nil, nil
	}
	...
	for _, line := range lines[1:] {
		...
		if len(fields) < 4 {
			return nil, errors.New("socket table row is incomplete")
		}
		// 0A is LISTEN; a zero remote endpoint is an unconnected UDP socket.
		if fields[3] == "0A" {
			continue
		}
		local, err := parseProcEndpoint(fields[1])
		if err != nil {
			return nil, err
		}
		remote, err := parseProcEndpoint(fields[2])
		if err != nil {
			return nil, err
		}
		remoteIP := net.ParseIP(remote.Address)
		if remote.Address == "" || remote.Port == 0 || remoteIP == nil || remoteIP.IsUnspecified() {
			continue
		}
...
func parseProcEndpoint(value string) (*antiflockv1.FlowEndpoint, error) {
	...
	if len(parts) != 2 {
		return nil, errors.New("socket endpoint is malformed")
	}
	...
	if err != nil {
		return nil, errors.New("socket port is invalid")
	}
