...
func TestLoadNodeCertificateUsesSameEnrolledSeed(t *testing.T) {
	...
	certPath := filepath.Join(directory, "node.pem")
	if err := os.WriteFile(seedPath, privateKey.Seed(), 0o600); err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC()
	...
	if err != nil || len(certificate.Certificate) != 1 || certificate.Leaf == nil {
