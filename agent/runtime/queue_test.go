...
func TestQueuePersistsSignedWireUntilAcknowledged(t *testing.T) {
	...
	if err != nil || len(batch) != 1 || batch[0].GetId() != event.GetId() {
	...
	if err != nil || len(batch) != 0 {
		t.Fatalf("ack left queued events: %#v %v", batch, err)
	}
}
...
func TestQueueDoesNotMixBootIDsInOneCoreBatch(t *testing.T) {
	...
	if err != nil || len(batch) != 1 || batch[0].GetBootId() != "boot-old" {
		t.Fatalf("batch=%#v err=%v", batch, err)
	}
	if err := queue.Acknowledge(context.Background(), batch); err != nil {
		t.Fatal(err)
	...
	batch, err = queue.Batch(context.Background(), 256)
	if err != nil || len(batch) != 1 || batch[0].GetBootId() != "boot-new" {
...
func TestQueueBatchAdmissionDoesNotPersistPartialCycleWhenFull(t *testing.T) {
	...
	if err := queue.EnqueueBatch(context.Background(), entries); err == nil {
		t.Fatal("full queue accepted a partial collection cycle")
	}
	if len(queue.state.Events) != maximumQueueEvents {
		t.Fatalf("queue size = %d, want %d", len(queue.state.Events), maximumQueueEvents)
	}
}
...
func TestQueueExcludesConcurrentWritersAndReleasesOnClose(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("queue did not release writer lock: %v", err)
	}
	defer second.Close()
}
...
func TestInspectQueueReadsOnlySafeMetadataWhileWriterIsActive(t *testing.T) {
	...
	if err := queue.Enqueue(context.Background(), &antiflockv1.EventEnvelope{Id: "event-status", NodeId: "node-1", BootId: "boot-1", Sequence: sequence}, collectors.QueuePriorityObservation); err != nil {
		t.Fatal(err)
	}
