...
func LoadFileSigner(nodeID, path string, clock func() time.Time) (collectors.EventSigner, error) {
	if strings.TrimSpace(nodeID) == "" { return nil, errors.New("agent signer requires node id") }
...
func LoadNodeCertificate(certificatePath, seedPath string) (tls.Certificate, error) {
	...
	if err != nil { return tls.Certificate{}, errors.New("read node certificate file") }
	values := make([][]byte, 0, 1)
	for rest := content; len(rest) != 0; {
		block, remaining := pem.Decode(rest)
		if block == nil { if strings.TrimSpace(string(rest)) == "" { break }; return tls.Certificate{}, errors.New("node certificate file contains invalid PEM") }
		if block.Type == "CERTIFICATE" { if _, err := x509.ParseCertificate(block.Bytes); err != nil { return tls.Certificate{}, errors.New("node certificate is invalid") }; values = append(values, block.Bytes) }
		rest = remaining
	}
...
func loadNodePrivateKey(path string) (ed25519.PrivateKey, error) {
	if strings.TrimSpace(path) == "" { return nil, errors.New("agent key file is required") }
