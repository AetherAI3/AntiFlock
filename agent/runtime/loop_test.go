...
func TestRunRetriesContinuousCollectionAfterTransientFailure(t *testing.T) {
	...
	loop, err := NewLoop(LoopConfig{
		...
		Submitter: submitterFunc(func(context.Context, *antiflockv1.SubmitEventBatchRequest) (*antiflockv1.EventBatchAck, error) {
	...
	if err := loop.Run(ctx); err != nil {
		t.Fatalf("continuous retry returned %v", err)
	}
	if calls != 2 {
		t.Fatalf("collection attempts = %d, want 2", calls)
	}
}
