...
func hostileJSONCases(t testing.TB) map[string]struct {
	...
	return map[string]struct {
		...
		"duplicate key by case": {append(append([]byte{}, withoutBrace...), []byte(`,"NODEID":"node-0002"}`)...), ReasonJSONDuplicateKey},
		"duplicate nested case": {bytes.Replace(canonical, []byte(`"probeDigest"`), []byte(`"KEY":"x","probeDigest"`), 1), ReasonJSONDuplicateKey},
...
func TestPrescanJSONStateMachine(t *testing.T) {
	...
	reject := map[string]string{
		...
		`{"a":1,"A":2}`:                       ReasonJSONDuplicateKey,
		`{"nodeId":"a","NODEID":"b"}`:         ReasonJSONDuplicateKey,
