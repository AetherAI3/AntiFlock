...
| BYOK providers | Headscale uses a locally stored read-only private key file plus explicit provider-to-node associations; Tailscale uses local CLI status and needs no provider credential. Browser code never receives either. | Key rotation/revocation, platform-keystore references, outbound egress controls, durable audit records, provider failure tests, and live integration coverage. |
