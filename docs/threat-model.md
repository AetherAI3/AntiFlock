...
| Telemetry or audit tampering | Per-node boot/sequence identity, deterministic protobuf signatures/digests, immutable append, transactional audit-head CAS, and a separately signed local anchor journal | A fully compromised node can lie before signing. Coordinated rollback of both SQLite and the local anchor requires an external witness to detect. |
