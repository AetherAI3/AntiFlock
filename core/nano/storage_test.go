...
func TestStorageCursorStoreSurvivesReopen(t *testing.T) {
	...
	store, err := nano.NewStorageCursorStore(database, func() time.Time { return now })
	if err != nil {
		t.Fatal(err)
	}
	digest := "sha256:012345678901234567890123456789012345678901234567890123456789abcd"
	advanced, err := store.CompareAndSwap(ctx, digest, "node-test", nano.Cursor{}, nano.Cursor{Initialized: true, NextDueUnix: 42})
	if err != nil || !advanced {
		t.Fatalf("cursor advance=%v err=%v", advanced, err)
	}
	...
	if err != nil || !cursor.Initialized || cursor.NextDueUnix != 42 {
...
func enrollNanoCursorNode(t *testing.T, database *storage.DB, now time.Time) {
	t.Helper()
	secret := sha256.Sum256([]byte("nano-cursor-token"))
	...
	if err := database.EnrollNode(context.Background(), secret[:], now, model.Node{ID: "node-test", Name: "Test", Type: "NODE_TYPE_AGENT", Platform: "linux", PlatformVersion: "test", Status: model.NodeActive, CapabilitiesVerification: "CLAIMED", PublicKey: []byte("key"), CertificatePEM: "certificate", EnrolledAt: now}); err != nil {
		t.Fatal(err)
	}
}
