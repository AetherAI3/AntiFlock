...
func (database *DB) CompareAndSwapNanoCursor(ctx context.Context, programDigest, nodeID string, previous, next NanoCursorRecord, now time.Time) (bool, error) {
	if !boundedNanoCursorID(programDigest, 128) || !boundedNanoCursorID(nodeID, 128) || now.IsZero() { return false, errors.New("nano cursor state is invalid") }
	previousInitialized, nextInitialized := 0, 0
	if previous.Initialized { previousInitialized = 1 }
	if next.Initialized { nextInitialized = 1 }
	...
	if err != nil { return false, fmt.Errorf("compare nano cursor: %w", err) }
	changed, err := result.RowsAffected()
	if err != nil { return false, fmt.Errorf("inspect nano cursor compare: %w", err) }
	if changed == 1 { return true, nil }
	if previous.Initialized || previous.NextDueUnix != 0 { return false, nil }
	result, err = database.db.ExecContext(ctx, `
		INSERT INTO nano_runner_cursors(program_digest, node_id, initialized, next_due_unix, updated_at)
		VALUES (?, ?, ?, ?, ?)
		ON CONFLICT(program_digest, node_id) DO NOTHING
	`, programDigest, nodeID, nextInitialized, next.NextDueUnix, updatedAt)
	if err != nil { return false, fmt.Errorf("create nano cursor: %w", err) }
	changed, err = result.RowsAffected()
	if err != nil { return false, fmt.Errorf("inspect nano cursor creation: %w", err) }
