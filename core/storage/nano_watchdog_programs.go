...
func (database *DB) CreateNanoWatchdogProgramWithAudit(ctx context.Context, record NanoWatchdogProgramRecord, entry model.AuditEntry) error {
	...
	if err != nil {
		return fmt.Errorf("begin watchdog program admission: %w", err)
	}
	defer tx.Rollback()
	if err := createNanoWatchdogProgram(ctx, tx, record); err != nil {
		return err
	}
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit watchdog program admission: %w", err)
	}
	return nil
}

func createNanoWatchdogProgram(ctx context.Context, executor contextExecer, record NanoWatchdogProgramRecord) error {
	if !validNanoWatchdogRecord(record) {
		return errors.New("watchdog program record is invalid")
	}
...
func (database *DB) GetNanoWatchdogProgram(ctx context.Context, id string) (NanoWatchdogProgramRecord, error) {
	if !boundedNanoCursorID(id, 128) {
		return NanoWatchdogProgramRecord{}, ErrNodeNotFound
	}
...
func (database *DB) GetNanoWatchdogProgramByOperation(ctx context.Context, operationID string) (NanoWatchdogProgramRecord, error) {
	if !boundedNanoCursorID(operationID, 128) {
		return NanoWatchdogProgramRecord{}, ErrNodeNotFound
	}
	return scanNanoWatchdogProgram(database.db.QueryRowContext(ctx, `
		SELECT id, node_id, name, source, program_digest, binding_id, status, operation_id, created_at, updated_at
...
func (database *DB) ListNanoWatchdogPrograms(ctx context.Context, nodeID string) ([]NanoWatchdogProgramRecord, error) {
	if !boundedNanoCursorID(nodeID, 128) {
		return nil, errors.New("watchdog node id is invalid")
	}
	rows, err := database.db.QueryContext(ctx, `
		SELECT id, node_id, name, source, program_digest, binding_id, status, operation_id, created_at, updated_at
		FROM nano_watchdog_programs WHERE node_id = ? ORDER BY updated_at DESC, id`, nodeID)
	if err != nil {
		return nil, fmt.Errorf("list watchdog programs: %w", err)
	}
	defer rows.Close()
	result := make([]NanoWatchdogProgramRecord, 0)
	for rows.Next() {
		record, scanErr := scanNanoWatchdogProgram(rows)
		if scanErr != nil {
			return nil, scanErr
		}
...
func scanNanoWatchdogProgram(row rowScanner) (NanoWatchdogProgramRecord, error) {
	var record NanoWatchdogProgramRecord
	...
	if err != nil {
		return NanoWatchdogProgramRecord{}, fmt.Errorf("scan watchdog program: %w", err)
	}
	var parseErr error
	if record.CreatedAt, parseErr = parseTime(created); parseErr != nil {
		return NanoWatchdogProgramRecord{}, errors.New("watchdog program creation time is invalid")
	}
	if record.UpdatedAt, parseErr = parseTime(updated); parseErr != nil {
		return NanoWatchdogProgramRecord{}, errors.New("watchdog program update time is invalid")
	}
	if !validNanoWatchdogRecord(record) {
		return NanoWatchdogProgramRecord{}, errors.New("watchdog program state is invalid")
	}
	return record, nil
}
