...
func (server *Server) handleListWatchdogs(response http.ResponseWriter, request *http.Request) {
	if server.nano == nil {
		server.handleUnavailable(response, request)
		return
	}
	nodeID := strings.TrimSpace(request.URL.Query().Get("nodeId"))
	if nodeID == "" {
		writeAPIError(response, http.StatusBadRequest, "INVALID_ARGUMENT", "nodeId is required.", "", false)
		return
	}
	...
	result := make([]watchdogView, 0, len(values))
	for _, value := range values {
		result = append(result, projectWatchdog(value))
	}
...
func (server *Server) handleAdmitWatchdog(response http.ResponseWriter, request *http.Request) {
	...
	if err := decodeJSON(response, request, &body, 80<<10); err != nil {
	...
	if err != nil {
		server.writeDomainError(response, http.StatusBadRequest, err, body.OperationID)
		return
	}
