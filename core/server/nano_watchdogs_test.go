...
func TestWatchdogAdmissionAndRunAPI(t *testing.T) {
	...
	programID, _ := record["id"].(string)
	if programID == "" {
		t.Fatalf("admission response = %#v", record)
	}
...
func TestWatchdogRunOpenFindingsUsesCoreProjectionOnly(t *testing.T) {
	...
	if forged.Code != http.StatusBadRequest {
		t.Fatalf("forged Core finding run = %d %s", forged.Code, forged.Body.String())
	}
	run := runtime.request(t, http.MethodPost, "/v1/watchdogs/"+programID+"/run-open-findings", nil, true)
	if run.Code != http.StatusOK {
		t.Fatalf("core finding run = %d %s", run.Code, run.Body.String())
	}
	result := decodeObject(t, run)
	if result["evaluatedCount"] != float64(1) {
